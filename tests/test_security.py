...
from app.utils.imagen_validators import (validar_extension_archivo)
...
def test_password_encriptada():
    ...
    assert verify_password(hash_generado,password)
...
def test_subida_archivo_ejecutable():
    ...
    with pytest.raises(ValueError):
        validar_extension_archivo("virus.exe")
...
def test_subida_script_php():
    ...
    with pytest.raises(ValueError):
        validar_extension_archivo("shell.php")

def test_subida_archivo_contenido_malicioso():

    with pytest.raises(ValueError):
        validar_extension_archivo("malware.bat")